...
test.describe('Renfe ticket booking', () => {
    ...
    // test('Handle no tickets available scenario for future date', async()=> {
    ...
    //         await resultPage.page.waitForTimeout(3000);
            
    //         // Check if no availability message is visible
    //         await test.step('Verify noDispoIda element is visible', async () => {
    //             const noAvailabilityVisible = await resultPage.noDispoIda.isVisible().catch(() => false);
    //             expect(noAvailabilityVisible).toBeTruthy();
    ...
    //             const ticketsCount = await resultPage.getAvailableTicket();
    //             expect(ticketsCount.length).toBe(0);
